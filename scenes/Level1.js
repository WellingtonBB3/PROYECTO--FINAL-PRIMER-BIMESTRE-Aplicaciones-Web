...
export default class Level1 extends Phaser.Scene {
  ...
  spawnEnemy() {
    ...
    const spawnY = Phaser.Math.Clamp(groundY - Math.round(180 * uiScale), 50, groundY - 60);

    const enemyDisplayWidth = Math.round(250 * uiScale);
    const enemyDisplayHeight = Math.round(312 * uiScale);
    const enemyBodyWidth = Math.max(24, Math.round(enemyDisplayWidth * 0.24));
    const enemyBodyHeight = Math.max(60, Math.round(enemyDisplayHeight * 0.02));
